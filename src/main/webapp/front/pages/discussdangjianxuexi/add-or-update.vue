	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussdangjianxuexi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussdangjianxuexi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussdangjianxuexi`, this.ruleForm);
						}else{
							await this.$api.add(`discussdangjianxuexi`, this.ruleForm);
						}
						uni.setStorageSync('discussdangjianxuexiCleanType',true);
