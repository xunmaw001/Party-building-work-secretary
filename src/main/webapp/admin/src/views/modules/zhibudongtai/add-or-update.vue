    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="近期动态" prop="jinqidongtai">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.jinqidongtai" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jinqidongtai" label="近期动态" prop="jinqidongtai">
                    <span v-html="ruleForm.jinqidongtai"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.jinqidongtai = this.ruleForm.jinqidongtai.replace(reg,'../../../ssmx7950/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhibudongtai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhibudongtaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
