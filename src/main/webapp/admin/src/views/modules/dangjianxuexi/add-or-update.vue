<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.xiangguanshipin" label="相关视频" prop="xiangguanshipin">
          <file-upload
          tip="点击上传相关视频"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.xiangguanshipin" label="相关视频" prop="xiangguanshipin">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.xiangguanshipin)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="资料简介" prop="ziliaojianjie">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.ziliaojianjie" label="资料简介" prop="ziliaojianjie">
                    <span>{{ruleForm.ziliaojianjie}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dangjianxuexi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dangjianxuexiCrossAddOrUpdateFlag = false;
						 this.parent.search();
